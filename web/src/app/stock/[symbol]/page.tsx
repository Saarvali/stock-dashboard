      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold">
            {stock.symbol}{" "}
            <span className="text-gray-500 text-xl">— {stock.name}</span>
          </h1>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <div
              className={`mt-1 inline-block px-2 py-1 rounded ${chip(
                stock.changePct
              )}`}
            >
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <div className="space-x-2 mt-1">
              <span className={`px-2 py-1 rounded ${pill(above50)}`}>
                {above50 ? "Above 50D" : "Below 50D"}
              </span>
              <span className={`px-2 py-1 rounded ${pill(above200)}`}>
                {above200 ? "Above 200D" : "Below 200D"}
              </span>
            </div>
            <div className="text-xs text-gray-500 mt-2">
              SMA50: {stock.sma50.toFixed(2)} • SMA200: {stock.sma200.toFixed(2)}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <div className="text-xs text-gray-500 mt-2">
              From 52w High: {fmtPct(stock.dist52wHighPct)}
            </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <div className="text-sm text-gray-500">
              6-month vs {data.benchmark}
            </div>
            <div
              className={`text-xl font-semibold ${chip(
                stock.m6VsBenchmarkPct
              )}`}
            >
              {fmtPct(stock.m6VsBenchmarkPct)}
            </div>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <div className="text-sm text-gray-500">
              12-month vs {data.benchmark}
            </div>
            <div
              className={`text-xl font-semibold ${chip(
                stock.m12VsBenchmarkPct
              )}`}
            >
              {fmtPct(stock.m12VsBenchmarkPct)}
            </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <div
              className={`text-xl font-semibold ${chip(stock.newsSent * 100)}`}
            >
              {stock.newsSent.toFixed(2)}
            </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <div
              className={`text-xl font-semibold ${chip(
                stock.redditSent * 100
              )}`}
            >
              {stock.redditSent.toFixed(2)}
            </div>
            <div className="text-xs text-gray-500 mt-1">−1..+1 (mock)</div>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-500">
          All values are mock placeholders. We’ll wire real EOD data and
          sentiment next.
        </p>
      </div>
    </main>
