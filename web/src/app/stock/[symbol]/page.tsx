...
export default async function StockPage({ params }: { params: { symbol: string } }) {
  ...
  // symbol in URL may be URL-encoded, decode and use exact key
    ...
    <main className="min-h-screen px-6 py-10 bg-gray-50">
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold">{stock.symbol} <span className="text-gray-500 text-xl">— {stock.name}</span></h1>
          <Link href="/" className="text-sm underline">← Back to dashboard</Link>
        </div>

        <div className="text-gray-600">As of <span className="font-medium">{data.asOf}</span> • Benchmark: <span className="font-medium">{data.benchmark}</span></div>
